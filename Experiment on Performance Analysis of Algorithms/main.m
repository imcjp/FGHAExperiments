...
for i=1:n
    for j=1:m
        x=xn(i);
        y=yn(j);
        disp(sprintf('When x is %g and y is %g.\n\tUsed time of fast algorithm is %g¦Ìs.\n\tUsed time of original algorithm is %g¦Ìs.\n\tFast algorithm runs %g times faster than original algorithm.',x,y,tm1(i,j)*1e6,tm2(i,j)*1e6,ratio(i,j)));
    end
end
for i=1:n
    x=xn(i);
    disp(sprintf('When x is %g.\n\tAverage used time of fast algorithm is %g¦Ìs.\n\tAverage used time of original algorithm is %g¦Ìs.',x,avgXTm1(i)*1e6,avgXTm2(i)*1e6));
end
for i=1:m
    y=yn(i);
    disp(sprintf('When y is %g.\n\tAverage used time of fast algorithm is %g¦Ìs.\n\tAverage used time of original algorithm is %g¦Ìs.',y,avgYTm1(i)*1e6,avgYTm2(i)*1e6));
end
if sum(failLog1(:))>1
...
disp(sprintf('Average used time of fast algorithm is %g¦Ìs.',avgTm1*1e6));
disp(sprintf('Average used time of original algorithm is %g¦Ìs.',avgTm2*1e6));
